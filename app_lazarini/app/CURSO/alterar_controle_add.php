...
if(isset($_POST['nome_curso_custo']))
{
  ...
  $custo = $mysqli->real_escape_string($_POST['custo']);
    ...
    $curso_custo = $sql_query->fetch_assoc();
    $id_curso = $curso_custo['id_curso'];

      $sql_code = "UPDATE curso_custos SET nome_curso_custo='$nome_curso_custo', custo='$custo' WHERE id_curso_custos = '$id_curso_custos'";
      $sql_query = $mysqli->query($sql_code) or die("Falha na execução do código SQL: ". $mysqli->error);
      mysqli_close($mysqli);
...
include('../conexao.php');
    ...
    $sql_code = "SELECT * FROM curso_custos cc INNER JOIN curso c ON c.id_curso = cc.id_curso WHERE cc.id_curso_custos = '$id_curso_custos'";
    $sql_query = $mysqli->query($sql_code) or die("Falha na execução do código SQL: ". $mysqli->error);
...
if($sql_query->num_rows > 0){?>
	...
	<input type="submit" value="ALTERAR" class="entrar">
  	...
  	<a href="deletar_controle_add.php?id_curso_custos=<?php echo $id_curso_custos; ?>"><input type="submit" value="DELETAR" class="entrar"></a>
    <?php }else{
      ?>
			<div class="msg_erro">
