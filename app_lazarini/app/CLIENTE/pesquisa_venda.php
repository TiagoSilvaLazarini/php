...
<body>
  ...
  <script src="../CSS/mask.js"></script>
    ...
    if (!isset($_POST['busca_1'])) {
        ...
        $pesquisa_adm = $mysqli->real_escape_string($_POST['busca_adm']);
        $pesquisa_1 = $mysqli->real_escape_string($_POST['busca_1']);
        $pesquisa_2 = $mysqli->real_escape_string($_POST['busca_2']);
        if($pesquisa_1 != null){
            if($pesquisa_2 != null){
                ...
                INNER JOIN venda v ON v.id_cliente = c.id_cliente
                INNER JOIN produto_n p ON p.id_produto = v.id_produto
                WHERE a.nome_admin LIKE '%$pesquisa_adm%' AND v.data >= '$pesquisa_1' AND v.data <= '$pesquisa_2'
                ORDER BY c.nome_cliente";
            }else{
                $sql_code = "SELECT * FROM cliente c 
                INNER JOIN administrador a ON a.id_administrador = c.id_administrador
                INNER JOIN venda v ON v.id_cliente = c.id_cliente
                INNER JOIN produto_n p ON p.id_produto = v.id_produto
                WHERE a.nome_admin LIKE '%$pesquisa_adm%' AND v.data >= '$pesquisa_1'
                ORDER BY c.nome_cliente";
            }
        }else if($pesquisa_2 != null){
            $sql_code = "SELECT * FROM cliente c 
            INNER JOIN administrador a ON a.id_administrador = c.id_administrador
            INNER JOIN venda v ON v.id_cliente = c.id_cliente
            INNER JOIN produto_n p ON p.id_produto = v.id_produto
            WHERE a.nome_admin LIKE '%$pesquisa_adm%' AND v.data <= '$pesquisa_2'
            ORDER BY c.nome_cliente";
        }else{
            $sql_code = "SELECT * FROM cliente c 
            INNER JOIN administrador a ON a.id_administrador = c.id_administrador
            INNER JOIN venda v ON v.id_cliente = c.id_cliente
            INNER JOIN produto_n p ON p.id_produto = v.id_produto
            WHERE a.nome_admin LIKE '%$pesquisa_adm%'
            ORDER BY c.nome_cliente";
        }
    }
    ...
    ?>
        <div class="pesquisa">
            <form action=""method="POST">
                ...
                <input name="busca_adm" value="<?php if(isset($_POST['busca_adm'])) echo $_POST['busca_adm']; ?>" placeholder="pesquisar administrador" type="text">
                <?php } ?>
                <input name="busca_1" value="<?php if(isset($_POST['busca_1'])) echo $_POST['busca_1']; ?>" placeholder="pesquisar" type="date"min="2020-01-01"max="2200-12-30">
                <input name="busca_2" value="<?php if(isset($_POST['busca_2'])) echo $_POST['busca_2']; ?>" placeholder="pesquisar" type="date"min="2020-01-01"max="2200-12-30">
                <input type="submit" value="pesquisar" class="entrar">
            </form>
        </div>
        <div class="tira">
            <h2 class="h2">Lista de vendas</h2>
        </div>
  <div class="mostrar">
